...
import { E164_STRING_REGEX } from '../register-phone.util';
import {
  DayOfWeek,
  DominantFoot,
  PlayerPosition,
  SkillLevel,
} from '../../users/player-profile.enums';
...
export class RegisterDto {
  ...
  @ApiProperty({
    ...
    description:
      'E.164 international format: leading +, then 2–15 digits (country + national, no spaces).',
  })
  @Transform(({ value }: { value: unknown }) =>
    typeof value === 'string' ? value.trim() : value,
  )
  ...
  @ApiPropertyOptional({
    description: 'Profile photo URL (https recommended)',
